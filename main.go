...
// NOTE: In this setup, we don't have a size for our slice fields in our structs.
// In this case that slice field is completely ignored by the toml encoder. Check
// file myfile-2.toml for the output.
...
func main() {
	...
	err := ioutil.WriteFile("myfile-2.toml", buf.Bytes(), 0666)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("created toml file myfile-2.toml")
}
